...
namespace Wasm2CIL
{
    ...
    public class WebassemblyFunc
    {
        ...
        public MethodInfo Emit (WebassemblyFunctionType type, TypeBuilder tb, string name, bool exported)
        {
            var param_types = type.EmitParams ();
            var return_type = type.EmitReturn ();
            var method = tb.DefineMethod (name, exported ? MethodAttributes.Public : MethodAttributes.Private, return_type, param_types);
    ...
    public class Parser
    {
        ...
        public void Emit (string outputName)
        {
            ...
            TypeBuilder tb = mb.DefineType (assemblyName + "." + outputName, TypeAttributes.Public, typeof (WebassemblyModule));
            // Compute imports
            var importTypes = new List<Type> ();
            if (imports != null)
            {
                var query = from import in imports
                            group import by import.module into grouped
                            select new
                            {
                                Name = grouped.Key,
                                Imports = from import in grouped
                                          where import.kind == 0x0
                                          select new { Index = import.index, Name = import.name }
                            };

                foreach (var mod in query)
                {
                    TypeBuilder tbImport = mb.DefineType (assemblyName + "." + outputName + ".Imports." + mod.Name, TypeAttributes.Public | TypeAttributes.Abstract);
                    foreach (var func in mod.Imports)
                    {
                        var fn = exprs [func.Index];
                        var type = types [fn_to_type [func.Index]];
                        fn.Emit (type, tbImport, func.Name, true);
                    }
                    importTypes.Add (tbImport.CreateType ());
                }
            }

            // Add imports as ctor arguments
            var constructor = tb.DefineConstructor (MethodAttributes.Public, CallingConventions.Standard, importTypes.ToArray ());
            ...
            {
                // Skip imports
                if ((from import in imports
                     where import.index == (ulong)i
                     select import).Any ())
                    continue;
                ...
                var emitted = fn.Emit (type, tb, fn_name, export_table.ContainsKey (i));
            }
